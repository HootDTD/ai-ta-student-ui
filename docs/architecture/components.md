...
- **`ApolloTopBar`** (default) — props `{ classId?, progress?, onBack?, backLabel?, onToggleSidebar?, hideProgressLink?, actions?, maxWidthClassName? }`. Renders Hoot's own `.site-header`/`.site-brand` classes directly. Left cluster: sidebar-toggle button (browse only) or back button (session/progress), then a class-switcher dropdown — fetches `listMyClasses()` on mount (independent of the `classId` prop, so it still renders on "no class in URL" error screens) and `router.push`es to `/apollo?class={id}` on selection, dropping any session/concept state. Right cluster: optional XP stat, then any `actions` (visible `.apollo-topbar__action` buttons — the session view passes "Understanding" (toggles the KG drawer) + "Start over" here rather than hiding them in the menu), then the `⋮` overflow menu (just "My progress" unless `hideProgressLink`, + "Return to Hoot"). All `.dropdown-item`s are `display:block` so `<Link>` and `<button>` rows are uniform width. Consumed by `ApolloBrowse`, `ApolloPageClient`, `ProgressClient`.
