...
- **`ApolloChat`** (default) — props `{ sessionId, initialMessages: {role, content}[], onKgUpdate(kg), onDoneClicked(), onDoneFromChat?(result: DoneResponse), disabled? }`. Owns local `messages`/`draft`/`sending`/`error` state. `handleSend` calls `sendChat(sessionId, msg)`; appends Apollo's reply, calls `onKgUpdate(resp.kg)`, and if `resp.intent_executed?.intent === "done"` forwards the embedded `DoneResponse` via `onDoneFromChat` (the chat-affirmed-done shortcut). On error it pops the optimistic student turn and renders `ApolloErrorSurface` inline. Renders as a fill-height flex column (`.apollo-chat`): a scrolling message area (`.apollo-chat__scroll`) over a bottom-pinned composer (`.apollo-chat__composer`), mirroring Hoot's chat shell — this only works because `ApolloPageClient` wraps the session in `.apollo-session-shell` (flex column, `100dvh`). Before the first turn the scroll area shows the centered Hoot owl (`<OwlVideo>`) + "I'm listening…"; the composer footer has Send + "I'm done teaching" (→ `onDoneClicked`). Both student and Apollo turns render their `content` through the shared `MathMarkdown` component (wrapped in a `.prose.md-body` div), so the LaTeX Apollo emits — e.g. `\(\sum...\)` — displays as typeset KaTeX instead of raw text. Consumed by `ApolloPageClient`.
...
- The shared `components/MathMarkdown.tsx` (`react-markdown` + `remark-math` + `rehype-katex` + `normalizeMath`) renders full markdown+math for `ApolloChat` turns, `ApolloProblemPanel`, and the main-chat assistant messages in `app/page.tsx`. Note the repo now has three math renderers by design: this markdown pipeline; `react-katex` `InlineMath` for `$..$`-only segments in `ApolloReportPanel`/`ApolloKGPanel`; and `mdToHtml` for the client-side printable report — do not conflate them.
