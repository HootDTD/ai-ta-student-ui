...
- **`ApolloBrowse`** (default) — props `{ classId, onStarted(sessionId) }`. Fetches `listConcepts`/`listProblems`, renders `ApolloTopBar` + `ApolloSidebar` in a real two-column `.apollo-layout` (sidebar is a persistent full-height page-side column on desktop, an off-canvas drawer on mobile — see Non-obvious conventions) with a centered welcome state (the Hoot owl `/thinking.mp4` loop + a "What are we teaching today?" title + "Pick a concept…" prompt) until one is chosen, then a difficulty tablist + problem cards ("Start teaching" → `startSession` → `onStarted`).
