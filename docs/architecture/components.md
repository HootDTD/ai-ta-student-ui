...
- **`ApolloChat`** (default) — props `{ sessionId, initialMessages: {role, content}[], onKgUpdate(kg), onDoneClicked(), onDoneFromChat?(result: DoneResponse), disabled? }`. Owns local `messages`/`draft`/`sending`/`error` state. `handleSend` calls `sendChat(sessionId, msg)`; appends Apollo's reply, calls `onKgUpdate(resp.kg)`, and if `resp.intent_executed?.intent === "done"` forwards the embedded `DoneResponse` via `onDoneFromChat` (the chat-affirmed-done shortcut). On error it pops the optimistic student turn and renders `ApolloErrorSurface` inline. Before the first turn the scrollback is empty (no prompt card — the problem card's goal line + the composer placeholder carry the instruction); footer has Send + "I'm done teaching" (→ `onDoneClicked`). Consumed by `ApolloPageClient`.
