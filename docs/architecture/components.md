...
- **`ApolloProblemPanel`** (default) — props `{ problem: ApolloProblem | null }`. Shows `problem_text` and a plain-muted "Teach Apollo enough to solve for {target_unknown}" goal line (no difficulty eyebrow — it was redundant with the browse difficulty tabs). Null ⇒ "No problem loaded yet." Consumed by `ApolloPageClient`.
